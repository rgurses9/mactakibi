...
const AdminPanel: React.FC<AdminPanelProps> = ({ isOpen, onClose, currentUserEmail }) => {
    ...
    return (
        ...
        >
            <div
                className="bg-white dark:bg-slate-900 rounded-xl shadow-2xl max-w-5xl w-full h-[80vh] flex flex-col overflow-hidden animate-in fade-in zoom-in-95 duration-200 border border-transparent dark:border-slate-800"
            ...
            >
                ...
                {/* Content */}
                <div className="flex-1 p-6 overflow-hidden flex flex-col bg-gray-50 dark:bg-slate-950">
                    ...
                    <div className="flex flex-col md:flex-row justify-between items-center gap-4 mb-6">
                        ...
                        <div className="relative w-full md:w-64">
                            ...
                            <input
                                ...
                                className="w-full pl-9 pr-4 py-2 rounded-lg border border-gray-200 dark:border-slate-800 bg-white dark:bg-slate-900 text-sm focus:ring-2 focus:ring-red-500 outline-none text-gray-800 dark:text-white"
                            />
                        </div>
                    </div>

                    <div className="flex-1 overflow-y-auto bg-white dark:bg-slate-900 rounded-xl border border-gray-200 dark:border-slate-800 shadow-sm custom-scrollbar">
                        {loading ? (
                                        ...
                                        filteredUsers.map(user => {
                                            ...
                                            return (
                                                <tr key={user.uid} className="hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                                                    ...
                                                    <td className="p-4">
                                                        {isAdmin ? (
                                                            <span className="inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-bold bg-red-50 dark:bg-red-900/20 text-red-700 dark:text-red-300 border border-red-100 dark:border-red-900/40">
                                                                <ShieldCheck size={12} /> Admin
                                                            </span>
                                                        ) : user.isApproved ? (
                                                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-300 border border-green-100 dark:border-green-900/40">
                                                                Onaylı
                                                            </span>
                                                        ) : (
                                                            <span className="inline-flex items-center px-2.5 py-1 rounded-full text-xs font-bold bg-yellow-50 dark:bg-yellow-900/20 text-yellow-700 dark:text-yellow-300 border border-yellow-100 dark:border-yellow-900/40">
                                                                Bekliyor
                                                            </span>
                                                        )}
                                                    </td>
